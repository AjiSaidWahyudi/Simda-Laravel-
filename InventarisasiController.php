...
class InventarisasiController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            'kartu_ruang_id' => 'required|exists:kartu_ruang,id',
            'kode_barang' => 'required|unique:inventarisasi,kode_barang,' . $id,
            'kode_register' => 'required',
            'jenis_barang' => 'required',
            'nama_pemegang' => 'required',
            'merek_tipe' => 'required',
            ...
            'harga' => 'required|numeric',
            'keterangan' => 'required',
            'existing_images' => 'nullable|array', // Nama file gambar lama yang dipertahankan
        ...
        $inventarisasi->update(
            collect($validated)->except(['gambar', 'existing_images'])->toArray()
        );

        // 2. TANGANI PENGHAPUSAN GAMBAR LAMA
        // Ambil semua gambar yang ada di DB saat ini
        $currentImagesInDb = $inventarisasi->gambar_inv; 
        $keptImages = $request->input('existing_images', []); // Array nama file dari hidden input JS

        foreach ($currentImagesInDb as $oldImg) {
            // Jika gambar di DB tidak ada dalam daftar yang dipertahankan (keptImages)
            if (!in_array($oldImg->gambar, $keptImages)) {
                // Hapus file fisik
                $filePath = public_path('gambar_barang/' . $inventarisasi->id . '/' . $oldImg->gambar);
        ...
        // 3. TANGANI TAMBAH GAMBAR BARU
        if ($request->hasFile('gambar')) {
            $existingCount = $inventarisasi->gambar_inv()->count(); // Hitung sisa gambar setelah penghapusan
        ...
        // 4. REGENERATE QR (Logika Anda sudah benar)
        $qrPath = public_path('qr_codes');
